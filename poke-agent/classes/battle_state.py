...
class BattleStateBuilder:
    ...
    def get_turn_history(self, n: int = 5) -> str:
        """Returns a readable summary of the last n turns parsed from battle observations."""
        observations = self.battle.observations
        if not observations:
            return "No turn history available yet."

        player_role = self.battle.player_role  # "p1" or "p2"

        sorted_turns = sorted(observations.keys())
        recent_turns = sorted_turns[-n:]

        turn_summaries = []
        for turn_num in recent_turns:
            obs = observations[turn_num]
            events = obs.events
            lines = []
            ...
            for event in events:
                ...
                if event_type == "move":
                    ...
                    if len(event) >= 4:
                        ...
                        lines.append(
                            f"  {target} took damage ({hp} HP remaining){source}"
                        )
                    ...
                    if len(event) >= 4:
                        side = (
                            "Your" if event[2].startswith(player_role) else "Opponent's"
                        )
